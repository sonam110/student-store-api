...
class ContestController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try
        {
            ...
            else
            {
                ...
                $contest->winner_prizes                			= (!empty($request->winner_prizes)) ? json_encode($request->winner_prizes, JSON_UNESCAPED_UNICODE) : null;
    ...
    public function update(Request $request, Contest $contest)
    {
        ...
        if($contest->status=='hold')
        {
            ...
            if(strtotime($start_date)<strtotime(date('Y-m-d', strtotime("+8 days"))))
            {
                return response()->json(prepareResult(true, 'The Contest / Event is on hold stage so you have to change the start date. The start date must be greater than or equal to 8 days from the current date.', getLangByLabelGroups('messages','message_contest_start_date_need_to_change')), config('http_response.success'));
            }
        }
    ...
    public function contestApplications(Request $request, $id)
    {
    	try
    	{
    	    ...
    	    {
    	        $contestApplications = ContestApplication::where('contest_id',$id)->orderBy('created_at','DESC')
                ->where('contest_applications.payment_status', 'paid')
    ...
    public function contestFilter(Request $request)
    {
        try
        {
            ...
            {
                $contests->inRandomOrder('contests.id');
            }
