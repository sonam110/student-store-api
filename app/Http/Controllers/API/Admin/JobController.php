...
class JobController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try
        {
            ...
            $job->nice_to_have_skills           = (!empty($request->nice_to_have_skills)) ? json_encode($request->nice_to_have_skills) : null;
            ...
            $job->job_environment               = (!empty($request->job_environment)) ? json_encode($request->job_environment) : null;
            $job->years_of_experience           = $request->years_of_experience;
            $job->known_languages               = (!empty($request->known_languages)) ? json_encode($request->known_languages) : null;
            ...
            $job = Job::with('user:id,first_name,last_name,gender,dob,email,contact_number,profile_pic_path,profile_pic_thumb_path','jobTags:id,job_id,title','addressDetail','categoryMaster','subCategory')
    ...
    public function update(Request $request, Job $job)
    {
        ...
        { 
            ...
            $job->job_nature                    = $request->job_nature;
            $job->job_hours                     = $request->job_hours;
            $job->job_environment               = (!empty($request->nice_to_have_skills)) ? json_encode($request->job_environment) : null;
            $job->years_of_experience           = $request->years_of_experience;
            $job->known_languages               = (!empty($request->nice_to_have_skills)) ? json_encode($request->known_languages) : null;
            $job->job_start_date                = $job_start_date;
            $job->application_start_date        = $application_start_date;
            $job->application_end_date          = $application_end_date;
            $job->is_published                  = $request->is_published;
